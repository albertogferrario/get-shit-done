...
- This contains context from `/conductor:discuss-milestone`
...
**If MILESTONE-CONTEXT.md exists (from /conductor:discuss-milestone):**
...
The main ROADMAP.md file holds full phase details for the active milestone. Archive files in `milestones/` are created only when a milestone ships (via `/conductor:complete-milestone`).
...
Plans:
- [ ] [N]-01: TBD (run /conductor:plan-phase [N] to break down)
...
This file was a handoff artifact from `/conductor:discuss-milestone`. Now that the milestone is created, the context is persisted in ROADMAP.md and the temporary file is no longer needed.
</step>
